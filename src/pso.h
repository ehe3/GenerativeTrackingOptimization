...
class PSO {
	...
	private:	
		...
		GLuint quadVAO, quadVBO, repeatQuadVAO, repeatQuadVBO, refdepthtex, peng, repeattex, ping, depthtexture, pong, difftex, pang, tex320x240, pung, tex160x120, pling, tex80x60, plang, tex40x30, plong, tex20x15, plung, tex5x5, pleng, tex1x1;
		// instance buffers
	...
	public:
		...
		{	
			...
			for (int generation = 0; generation < iters; generation++)
			{
				...
				glGetTextureImage(tex1x1, 0, GL_DEPTH_COMPONENT, GL_FLOAT, sizeof(float)*NumParticles, currentdt);
				...
				for (int p = 0; p < NumParticles; p++)
				{
					if (currentdt[p] < particles[p].BestEnergyScore)
					{
						...
						std::cout << "cie: " << currentdt[p] << std::endl;
